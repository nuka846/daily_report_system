...
public abstract class ActionBase {
    ...
    /*
     * 初期化処理
     * サーブレットコンテキスト、リクエスト、レスポンスを暮らすフィールドに設定
    ...
    protected void invoke()
        ...
        try {
            ...
            //(例：action~Employee command=show の場合EmployeeActionクラスのshow()メソッドを実行する)
            commandMethod = this.getClass().getDeclaredMethod(command,new Class[0]);
            commandMethod.invoke(this,new Object[0]); //メソッドにわたす引数は無し
     ...
     * @return true:token友好　false:token不正
     * @throws ServletException
     * @throws IOException
     */
     ...
     */
    @SuppressWarnings("unchecked")
    protected <R> R getConstextScope(PropertyConst key) {
